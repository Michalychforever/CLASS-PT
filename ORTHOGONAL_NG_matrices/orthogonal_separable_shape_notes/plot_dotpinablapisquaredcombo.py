...
with open("dotpinablapisquaredcombo.txt","r") as file:
    expr = file.read()
...
x1_range=np.linspace(0+epsilon,1-epsilon,1000)
x2_range=np.linspace(1-epsilon,0.5+epsilon,1000)
...
for el in dict_labels:
    ...
    plt.title("$S_{{\\dot\\pi}(\\bm{\\nabla}\pi)^2}$, normalized to $S(k,k,k)=1$",y=1.01,fontsize=17)
