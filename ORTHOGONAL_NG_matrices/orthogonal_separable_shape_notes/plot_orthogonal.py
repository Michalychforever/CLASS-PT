...
with open("orthogonal.txt","r") as file:
    expr = file.read()

mathexpr = mathematica(expr)

shape=lambdify([x1,x2],mathexpr)#,"mpmath")

#--> could be that the mpmath issue is related to epsilon being not defined as mpf or something? Whatever...
...
x1_range=np.linspace(0+epsilon,1-epsilon,1000)#0)
x2_range=np.linspace(1-epsilon,0.5+epsilon,1000)#0)
...
for el in dict_labels:
    ...
    plt.title("$S_{\\rm ortho}$, normalized to $S(k,k,k)=1$",y=1.01,fontsize=17) #--> it was 1.025...
