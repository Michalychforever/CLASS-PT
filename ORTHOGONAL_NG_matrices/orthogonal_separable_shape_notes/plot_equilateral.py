...
#@np.vectorize
def S(x1,x2):
    numerator = x1*np.power(x2-1,2) + np.power(x1,2)*(x2+1) - np.power(x1,3) - (1+x2)*np.power(x2-1,2)
    denominator = x1*x2
...
for el in dict_labels:
    ...
    plt.title("$S_{\\rm equil}$, normalized to $S(k,k,k)=1$",y=1.01,fontsize=17) #--> it was 1.025...
