...
class_alloc(pnlpt->nl_corr_density,pnlpt->tau_size*ppt->k_size[pnlpt->index_md_scalars]*sizeof(double),pnlpt->error_message);
    ...
    //  double deltatau[pnlpt->z_pk_num];
      ...
      printf("Ok here...\n"); //GC!!! I had troubles importing stuff...
...
//printf("tau cycle is run over %d ms\n",end1-start1);
      ...
      class_call(nonlinear_pt_pPRIMk_l(//pba,
                                       ...
                                       pPRIMk_l,lnpPRIMk_l,ddlnpPRIMk_l),pnlpt->error_message,
                 pnlpt->error_message);
...
int start=clock();
            ...
            //printf("~> fine so far => entering nonlinear_pt_loop...\n"); //GC!
   ...
   //  printf("deltaSigma_BAO(ks=0.2 h/Mpc)=%lf (Mpc/h)^2\n",deltaSigmaBAO * pow(pba->h,2.));
     ...
     //   IR-5) Computing the LO IR resummed power spectrum
         ...
         for (index_kdisc=0; index_kdisc<Nmax; index_kdisc++){
            ...
            //     Pw[index_kdisc] = 0.;
                 //Pbin[index_kdisc] = Pnw[index_kdisc] + Pw[index_kdisc] * exp(-1000.*SigmaBAO * pow(kdisc[index_kdisc],2.)); //GC!
                 Pbin[index_kdisc] = Pnw[index_kdisc] + Pw[index_kdisc] * exp(-SigmaBAO * pow(kdisc[index_kdisc],2.));
                 //Tbin[index_kdisc] = Tnw[index_kdisc]/kdisc[index_kdisc]/kdisc[index_kdisc] + Tw[index_kdisc] * exp(-1000.*SigmaBAO * pow(kdisc[index_kdisc],2.))/kdisc[index_kdisc]/kdisc[index_kdisc]; //GC!
                 //Tbin[index_kdisc] = Tnw[index_kdisc]/kdisc[index_kdisc]/kdisc[index_kdisc] + Tw[index_kdisc] * exp(-SigmaBAO * pow(kdisc[index_kdisc],2.))/kdisc[index_kdisc]/kdisc[index_kdisc]; //GC! i_kdisc and index_kdisc have the same range... This should be correct... Here I divide by the k^2. The factors must still be fixed...
             ...
             //printf("%f",pow(Pdisc[index_kdisc]/PPRIMdisc[index_kdisc],0.5)/kdisc[index_kdisc]/kdisc[index_kdisc]);
             //printf("%f",PPRIMdisc[index_kdisc]);
             
             //printf("%lf, %lf",kdisc[index_kdisc],PPRIMdisc[index_kdisc]);
             //printf("\n");
         ...
         Ptree[index_kdisc] = Pnw[index_kdisc] + Pw[index_kdisc] * exp(-SigmaBAO * pow(kdisc[index_kdisc],2.))*(1. + SigmaBAO * pow(kdisc[index_kdisc],2.));
             //GC! Above I do my thing with square roots...
         //Ttree[index_kdisc] = .../PPRIMdisc[index_kdisc]; //GC! NOT NEEDED!!! THIS IS THE TIMES (1+\Sigma^2k^2) WHICH I DO NOT NEED!!!
     
             //printf("%.16f %.16f",kdisc[index_kdisc],Tbin[index_kdisc]);
             //printf("\n");

             //printf("%.24f %.24f",kdisc[index_kdisc],Tw[index_kdisc]);
             //printf("\n");
             
             //printf("%.24f %.24f",kdisc[index_kdisc],Tbin[index_kdisc]-Tnw[index_kdisc]);
             //printf("\n");
...
if (pnlpt->rsd == rsd_yes){ //GC: screwed up a bracket. Should end at the line "// end of RSD conditional expression"...
...
free(y);
     ...
     //printf("~> fine so far 1...\n");
     
free(Tbin); //GC!

     //printf("~> fine so far 2...\n");

free(y_transfer); //GC!
     
     //printf("~> fine so far 3...\n");
