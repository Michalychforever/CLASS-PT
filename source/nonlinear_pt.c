...
class_call(background_at_tau(pba,pnlpt->tau[index_tau],long_info,inter_normal,&last_indexD,pvecbackD),pnlpt->error_message,pnlpt->error_message);
    ...
    for (index_k=0; index_k<ppt->k_size[pnlpt->index_md_scalars]; index_k++) 
        ...
        pnlpt->nl_corr_density[index_tau * ppt->k_size[pnlpt->index_md_scalars] + index_k] = sqrt((pk_Tree_int[index_k]+Dplus*Dplus*(pk_nl_int[index_k]-5000.)/Dref/Dref)/pk_l_at_z_req_int[index_k]);
    } else {
    for (index_k=0; index_k<ppt->k_size[pnlpt->index_md_scalars]; index_k++) 
        pnlpt->nl_corr_density[index_tau * ppt->k_size[pnlpt->index_md_scalars] + index_k] = sqrt((pk_Tree[index_k]+Dplus*Dplus*(pk_nl[index_k]-5000.)/Dref/Dref)/pk_l_at_z_req[index_k]);
    }
    //printf("index_tau=%i \n",index_tau);
    //printf("index_k=%i \n",index_k);
 ...
 //        FFT(input_real_test,input_imag_test,output_real_test,output_imag_test,Nirby4,stepsize);
  ...
  // printf("deltaSigma_BAO(ks=0.2 h/Mpc)=%lf (Mpc/h)^2\n",deltaSigmaBAO * pow(pba->h,2.));
     ...
     //   IR-5) Computing the LO IR resummed power spectrum
         ...
         // printf("%le %le\n",kdisc[index_kdisc],Pnw[index_kdisc]-Ptree[index_kdisc]);
     }
