...
int nonlinear_pt_pk_l(
  ...
  char line[100000];
  FILE *fp;
  fp = fopen("/home/ophilcox/k_pk_to_interp.dat","r");
  if (fp==NULL){
      fprintf(stderr,"Interpolation file not found\n");
      abort();
  }
  ...
  // Now create an interpolator
  gsl_interp_accel *kaccel;
  gsl_spline *interp_kernel;

  interp_kernel = gsl_spline_alloc(gsl_interp_cspline,nline);
  gsl_spline_init(interp_kernel,kint,pkint,nline);
  kaccel = gsl_interp_accel_alloc();
  //OLIVER END
...
//printf("k=%f source_m=%f   source_cdmb=%f\n",pnlpt->k[index_k],ppt->sources[index_md][index_ic1 * ppt->tp_size[index_md] + ppt->index_tp_delta_m][index_tau * ppt->k_size[index_md] + index_k],source_ic1);
 ...
 //   printf("%e  %e\n",pnlpt->k[index_k],pk_l[index_k]);
  ...
  //OLIVER START - COMMENT THIS TO GET USUAL BEHAVIOR
 // Replace lnpk, pk_l with interpolated values
 double this_pk, this_k;
 
 this_k = pnlpt->k[index_k];
 if (this_k<kmin) this_pk = pkint[0];
 else if (this_k>kmax) this_pk = pkint[nline-1];
 else this_pk = gsl_spline_eval(interp_kernel,this_k,kaccel);
 pk_l[index_k] = this_pk;
 lnpk[index_k] = log(this_pk);
 //OLIVER END
}
