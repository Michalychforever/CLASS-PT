...
cosmo = Class ()
#cosmo.set({'k_pivot':'0.05','A_s':'2.19896453e-9','n_s':'0.95','alpha_s':'0.','Omega_k':'0.','Omega_fld':'0','YHe':0.25,'z_reio':'10.','T_cmb':'2.726','h':'0.7','Omega_b':'0.0469388','N_ncdm':'0','N_eff':'3.04','Omega_cdm':'0.223061','P_k_max_h/Mpc': '100.','output':'mPk','z_pk':'0.','non linear':' SPT ','IR resummation':' Yes ','Bias tracers':' Yes ','RSD':' Yes ','RSD only':'No'})

cosmo.set({'k_pivot':'0.05','ln10^{10}A_s':'3.045','n_s':'0.9649','alpha_s':'0.','Omega_k':'0.','Omega_fld':'0','YHe':0.25,'z_reio':'10.','T_cmb':'2.726','h':'0.6727','omega_b':'0.02236','N_ncdm':'0','N_eff':'3.04','omega_cdm':'0.1202','P_k_max_h/Mpc': '100.','output':'mPk','z_pk':'1.6827623897643107','non linear':' SPT ','IR resummation':' Yes ','Bias tracers':' Yes ','RSD':' Yes ','RSD only':'No'})
...
k = np.exp(k)
z = 1.6827623897643107
testout = [ [0 for x in range(50)] for y in range(len(k))];
for i in range(len(k)):
    testout[i][0] = k[i]
    testout[i][-1] = cosmo.pk_lin(k[i],z)
    for j in range(48):
#        print("j=",j)
