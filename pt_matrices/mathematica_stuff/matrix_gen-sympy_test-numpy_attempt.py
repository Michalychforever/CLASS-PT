...
with open("vv0_f2.txt","r") as file:
    expr = file.read()

mathexpr=mathematica(expr)

M12temp=lambdify([n1,n2],mathexpr,"numpy")#"mpmath")
...
def J(nu1,nu2):
	return (special.gamma(1.5 - nu1) * special.gamma(1.5 - nu2) * special.gamma(nu1 + nu2 - 1.5) / (special.gamma(nu1) * special.gamma(nu2) * special.gamma(3. - nu2 - nu1))) / (8. * np.pi**(1.5))
...
#print(-0.5*etam[1])
#print(M12(-0.5*etam[1],-0.5*etam[3]))
#print(M12(-0.5*(1+3*1j),-0.5*(1+3*1j)))
#print(J(-0.5*etam[1],-0.5*etam[1])) #--> do not know why it doesn't work...
#print(-0.5*etam[1],-0.5*etam[1])
#print(J(0.1*pi,0.1))
#--> it was conflict with the pi of numpy...
...
np.savetxt('M12oneline_N512-sympy_test-numpy_attempt.dat',moutoneline)
